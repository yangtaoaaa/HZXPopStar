...
+ (void)initialize{
    ...
    // 设置背景
    [navBar setBackgroundImage:[UIImage imageNamed:@"白色图片"] forBarMetrics:UIBarMetricsDefault];
    //[navBar setBackgroundColor:HZXColor(248, 248, 248, 1)];
        
    // 设置标题属性
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = HZXColor(33, 33, 33, 1);
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if(self.viewControllers.count > 0){
        ...
        //[self.navigationBar setBackgroundColor:HZXColor(248, 248, 248, 1)];
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithIcon:@"返回" highIcon:@"返回" target:self action:@selector(back)];
    }
